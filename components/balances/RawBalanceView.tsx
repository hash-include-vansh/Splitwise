...
function BalanceRow({
  ...
  const containerClass = isSettled
    ...
    : hasPaidAmount
      ? `w-full text-left rounded-xl border border-amber-300 bg-amber-50 p-4 shadow-elegant${isClickable ? ' transition-all hover:shadow-medium hover:border-amber-400 cursor-pointer active:scale-[0.98]' : ''}`
      : `w-full text-left rounded-xl border border-gray-200/60 bg-white p-4 shadow-elegant${isClickable ? ' transition-all hover:shadow-medium hover:border-gray-300/60 cursor-pointer active:scale-[0.98]' : ''}`
      ...
      <div className="flex items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <p
              className={`text-sm font-bold mb-0.5 truncate tracking-tight ${isSettled ? 'line-through text-green-700' : 'text-gray-900'}`}
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
          <div className="text-center">
            {hasPaidAmount ? (
              ...
              <div
                className="text-base sm:text-lg font-black text-green-600 tracking-tight line-through"
                style={{ letterSpacing: '-0.02em' }}
              >
          ...
          <ArrowRight
            className={`h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 ${isSettled ? 'text-green-400' : 'text-gray-400'}`}
          />
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            />
            <p
              className={`text-sm font-bold truncate tracking-tight hidden sm:block ${isSettled ? 'line-through text-green-700' : 'text-gray-900'}`}
              style={{ letterSpacing: '-0.01em' }}
            >
              {toUser?.name || toUser?.email || 'Unknown'}
            </p>
          </div>
          {isClickable && !isSettled && <ArrowRight className="h-4 w-4 text-gray-300" />}
        </div>
      </div>
      {hasPaidAmount && (
